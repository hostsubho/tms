...
Module licensing & theming adds a new `TenantModuleFlags` table plus `ModuleBillingTotalOverrideCents`/`SecondaryColor`/`AccentColor`/`ThemeMode`/`BorderRadius`/`Density`/`CustomCss` to `Tenants`:
`dotnet ef migrations add AddModuleLicensingAndTheming --output-dir Migrations && dotnet ef database update`
...
- `GET /me` — the caller's own tenant (name, subdomain, timezone, branding, plan, status, trial end), plus as of the module-licensing/theming feature: `enabledModules` (the tenant's own resolved list, one entry per `ModuleKey` currently on — same fallback logic as `IModuleAccessService`, see Module Licensing below) and the full theme payload (`secondaryColor`, `accentColor`, `themeMode`, `borderRadius`, `density`, `customCss`, alongside the pre-existing `primaryColor`). Any authenticated tenant user — the frontend dashboard nav reads `enabledModules` from here to decide which nav items to even show.
- `PATCH /me` — update name/timezone/branding. `Admin` only.
- `PATCH /me/theme` — `{ primaryColor?, secondaryColor?, accentColor?, themeMode?, borderRadius?, density?, customCss? }`. `Admin` only. All fields optional/partial-update; `themeMode`/`borderRadius`/`density` are validated against their enums server-side (400 with a clear message on an invalid value, e.g. `"Dim"` instead of `Light`/`Dark`/`Auto`) rather than silently coercing to a default. `customCss` is capped at 20,000 characters — a large but bounded escape hatch, not a place to store an entire alternate stylesheet. This is a **client-side reskin layer, not a true design-token system** — see the frontend's `dashboard/layout.tsx` for exactly which utility classes it overrides and why (the codebase's existing hardcoded-Tailwind-class architecture wasn't worth a full retrofit for this pass).
...
**Reports** (`/api/reports`) — Module 9, requires the `TenantStaff` policy (not reachable with a portal customer token); gated behind the `AdvancedReports` module flag (see Module Licensing below) — 403s with a clear message if a tenant has been licensed down
- `GET /dashboard` — one aggregate payload: `ticketVolume` (totals by status, plus a 30-day daily-created series), `slaCompliance` (breached count + `compliancePercentage` over tickets that had an SLA policy matched at all — a tenant with none tracked reads as 100%, not 0%), `teamPerformance` (team-wide roll-up: active agent count, total assigned/resolved/breached, and weighted-average resolution/first-response/CSAT + SLA compliance across the team — weighted by each agent's own ticket count, not a plain average-of-averages, so a 50-ticket agent doesn't count the same as a 2-ticket one), `agentPerformance` (per active agent: assigned/open/resolved/breached counts, average resolution and first-response time in hours, SLA compliance percentage, and average CSAT rating — the per-employee performance view), `csat` (average rating, total ratings, a 1–5 star distribution, and a 30-day daily-average series).
...
**Module Licensing & Billing Overrides** (`/api/platform/tenants/{id}/module-flags`, `/api/platform/tenants/{id}/billing-total-override`) — Super Admin, requires a platform token
- The "Owner can enable/disable modules per client and negotiate cost" feature. `ModuleKey` covers every optional module: `SlaPolicies`, `Automation`, `KnowledgeBase`, `AdvancedReports`, `Cmdb`, `IntegrationsApi`, `CustomRoles`, `Sso`. Core ticketing (tickets, comments, categories, users, notifications) is never gated — always on for every tenant.
- `GET /{id}/module-flags` — every module's current state for this tenant (`enabled`, optional `monthlyCostCents`), plus `basePlanPriceMonthlyCents`, `suggestedTotalCents` (base plan + sum of each enabled module's set cost), `totalOverrideCents` (null unless the Owner has negotiated a different number), and `effectiveTotalCents` (the override if set, else the suggested total). Any platform role (`PlatformAdmin` policy — read-only).
- `PUT /{id}/module-flags/{moduleKey}` — `{ enabled, monthlyCostCents? }`. Requires the `PlatformManage` policy (`Owner`/`PlatformAdmin` — enabling/disabling functionality is more than a pure billing action, unlike the override below). A tenant with no row yet for a given module defaults to **enabled** for every module except `Cmdb`, which falls back to the legacy `Tenant.CmdbEnabled` flag (default off) — so this feature ships with zero data backfill and doesn't silently unlock or break anything for tenants that predate it. Toggling `Cmdb` here also mirrors `Tenant.CmdbEnabled` for backward compatibility with the pre-existing Module 10 CMDB toggle. Audit-logged (`AuditEntityType.Billing`).
- `PUT /{id}/billing-total-override` — `{ totalOverrideCents? }`. Requires the `PlatformBilling` policy (`Owner`/`PlatformAdmin`/`BillingAdmin` — purely financial, same policy as billing credits). Sets or clears (pass `null`) a flat negotiated total that overrides `suggestedTotalCents` entirely. Audit-logged.
- **Deliberately not wired to Stripe or any automatic charge** — there's no fixed per-module price catalog in this iteration (per-module cost is whatever the Owner manually types in when toggling a module on for a given tenant), so this is a cost-tracking/negotiation tool for the Super Admin console, not a billing engine. Wiring `effectiveTotalCents` into an actual recurring Stripe charge is future work once real module pricing is decided.
- Enforcement: `IModuleAccessService.IsEnabledAsync(tenantId, key, ct)` is checked at the top of every gated controller action (`SlaPoliciesController`, `AutomationRulesController`, `KnowledgeArticlesController`, `ReportsController`, `AssetsController`, `ApiKeysController`+`WebhooksController` (share `IntegrationsApi`), `CustomRolesController`, `SsoConfigController`+`SsoAuthController`), returning a 403 with a plain-English message rather than a generic "forbidden." Scoped to gating the *management/admin API surface* per module, not deep runtime side effects — disabling Automation doesn't retroactively deactivate already-configured rules, disabling Custom Roles doesn't strip permissions already assigned to a user, disabling Integrations doesn't revoke already-issued API keys. A deliberate scope decision to avoid unpredictable behavior when an Owner licenses a client down.
